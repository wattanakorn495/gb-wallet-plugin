...
public class LivenessCheckProcessor extends Processor implements FaceTecFaceScanProcessor {
    ...
    public LivenessCheckProcessor(String sessionToken, Context context) {
        ...
        //
        // Required parameters:
        // - Context: Unique for Android, a Context is passed in, which is required for
        // the final onActivityResult function after the FaceTec SDK is done.
        // - FaceTecFaceScanProcessor: A class that implements FaceTecFaceScanProcessor,
        // which handles the FaceScan when the User completes a Session. In this
        // example, "self" implements the class.
        // - sessionToken: A valid Session Token you just created by calling your API to
        // get a Session Token from the Server SDK.
        //
        Log.e("FaceTecSessionActivity", "createAndLaunchSession Start ");
        FaceTecSessionActivity.createAndLaunchSession(context, LivenessCheckProcessor.this, sessionToken);
        Log.e("FaceTecSessionActivity", "createAndLaunchSession finish");
    }
    ...
    public void processSessionWhileFaceTecSDKWaits(final FaceTecSessionResult sessionResult,
        ...
        //
        // DEVELOPER NOTE: These properties are for demonstration purposes only so the
        // Sample App can get information about what is happening in the processor.
        // In the code in your own App, you can pass around signals, flags,
        // intermediates, and results however you would like.
        //
        // sampleAppActivity.setLatestSessionResult(sessionResult);
        GbkycPlugin.latestSessionResult = sessionResult;
        Log.e("processSessionWhileFaceTecSDKWaits", "Session Result" + String.valueOf(sessionResult.getStatus()));
        //
        ...
        // IMPORTANT: FaceTecSDK.FaceTecSessionStatus.SessionCompletedSuccessfully DOES
        // NOT mean the Liveness Check was Successful.
        ...
        //
        // Part 5: Make the Networking Call to Your Servers. Below is just example code,
        // you are free to customize based on how your own API works.
        //
        ...
        NetworkingHelpers.getApiClient().newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NonNull Call call, @NonNull okhttp3.Response response) throws IOException {
                ...
                //
                // Part 6: In our Sample, we evaluate a boolean response and treat true as was
                // successfully processed and should proceed to next step,
                // and handle all other responses by cancelling out.
                // You may have different paradigms in your own API and are free to customize
                // based on these.
                //
                ...
                try {
                    ...
                    // In v9.2.0+, we key off a new property called wasProcessed to determine if we
                    // successfully processed the Session result on the Server.
                    // Device SDK UI flow is now driven by the proceedToNextStep function, which
                    // should receive the scanResultBlob from the Server SDK response.
                    if (wasProcessed) {

                        // Demonstrates dynamically setting the Success Screen Message.
                        FaceTecCustomization.overrideResultScreenSuccessMessage = "Liveness\nConfirmed";

                        // In v9.2.0+, simply pass in scanResultBlob to the proceedToNextStep function
                        // to advance the User flow.
                        // scanResultBlob is a proprietary, encrypted blob that controls the logic for
                        // what happens next for the User.
                        success = faceScanResultCallback.proceedToNextStep(scanResultBlob);
                    } else {
                        // CASE: UNEXPECTED response from API. Our Sample Code keys off a wasProcessed
                        // boolean on the root of the JSON object --> You define your own API contracts
                        // with yourself and may choose to do something different here based on the
                        // error.
                        faceScanResultCallback.cancel();
                    }
                } catch (JSONException e) {
                    // CASE: Parsing the response into JSON failed --> You define your own API
                    // contracts with yourself and may choose to do something different here based
                    // on the error. Solid server-side code should ensure you don't get to this
                    // case.
                    e.printStackTrace();
            ...
            public void onFailure(@NonNull Call call, @Nullable IOException e) {
                // CASE: Network Request itself is erroring --> You define your own API
                // contracts with yourself and may choose to do something different here based
                // on the error.
